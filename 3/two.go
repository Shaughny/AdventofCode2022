...
input:= `BdbzzddChsWrRFbzBrszbhWMLNJHLLLLHZtSLglFNZHLJH
...
for i := 0;i < (len(rugsacks)); i +=3{
	m := make(map[string]int)

	for _, c := range rugsacks[i]{
		if m[string(c)] == 0{
			m[string(c)] = 1
		}
	}
	for _, c := range rugsacks[i + 1]{
	...
	if int(duplicate) > 95{
		priority += int(duplicate) - 96
	}else{
		priority += int(duplicate) - 38
	}
} 
